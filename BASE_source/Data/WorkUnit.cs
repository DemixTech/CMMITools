...
namespace BASE.Data
{
    public class WorkUnit
    {
        ...
        private void ProcessProjects(Worksheet prjWks, int row, int headingRow)
        {
            ...
            try
            {
                sValue2 = (string)prjWks.Cells[row, 1].Value; // ID field
                ...
                sValue2 = (string)prjWks.Cells[row, 2].Value; // Name
                if (string.IsNullOrEmpty(sValue2))
                {
                    WorkType = EWorkType.nothing;
                ...
                sValue2 = (string)prjWks.Cells[row, 3].Value; // Description
                if (string.IsNullOrEmpty(sValue2))
                {
                    WorkType = EWorkType.nothing;
                    return;
                ...
                sValue2 = (string)prjWks.Cells[row, 4].Value; // Lifecycle
                if (string.IsNullOrEmpty(sValue2))
                {
                    WorkType = EWorkType.nothing;
                    return;
                ...
                sValue2 = (string)prjWks.Cells[row, 5].Value; // Stage
                if (string.IsNullOrEmpty(sValue2))
                {
                    WorkType = EWorkType.nothing;
                    return;
                ...
                sValue2 = (string)prjWks.Cells[row, 6].Value.ToString(); // Start
                if (string.IsNullOrEmpty(sValue2))
                {
                    WorkType = EWorkType.nothing;
                    return;
                ...
                sValue2 = (string)prjWks.Cells[row, 7].Value.ToString(); // End
                if (string.IsNullOrEmpty(sValue2))
                {
                    WorkType = EWorkType.nothing;
                    return;
        ...
        {
            string sValue2;
            sValue2 = (string)prjWks.Cells[row, 1].Value; // ID
            if (string.IsNullOrEmpty(sValue2))
            {
                WorkType = EWorkType.nothing;
                return;
            ...
            for (int col = 6; col <= 26; col++)
            {
                ...
                try
                {
                    sValue2 = (string)prjWks.Cells[headingRow, col].Value.ToString().ToLower().Trim();  // identify the PA
                    EPAcode thePA = (EPAcode)Enum.Parse(typeof(EPAcode), sValue2, true);
                    PracticeArea aNewPA = new PracticeArea();
                    aNewPA.PAcode = thePA;
