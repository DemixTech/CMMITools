...
cls.prototype.handleConflictsOfInterestPage = async function(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      templateHeaders = stringMatches.slice(0, 6).map((m: any) => {
      ...
      templateHeaders = [
        '* Conflict of Interests', '* Other Conflict of Interest',
        '* Description', '* Impact', '* Management Strategy', '* Status'
      ];
    }

    const outWorkbook = new ExcelJS.Workbook();
    const outSheet = outWorkbook.addWorksheet('Conflicts of Interest');
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      row.getCell(1).value = d.conflict;
      row.getCell(2).value = d.otherConflict;
      row.getCell(3).value = d.description;
      row.getCell(4).value = d.impact;
      row.getCell(5).value = d.managementStrategy;
      row.getCell(6).value = d.status;
      console.log(`      Row ${i + 2}: "${d.conflict.substring(0, 45)}"`);
    }
