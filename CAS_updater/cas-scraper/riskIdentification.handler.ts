/**
 * Risk Identification Page Handler - Mixin applied to CASPopulator
 *
 * Page: /risk-identification
 * Workflow:
 *   1. Delete all existing entries (ConfirmDelete pattern)
 *   2. Download the template
 *   3. Populate from C_RiskIdentification sheet (headers match template)
 *   4. Upload the populated template
 *
 * Source: C_RiskIdentification sheet in target xlsm
 *   Row 1: headers (7 columns, identical to template)
 *   Row 2+: data rows copied directly (same column order)
 *
 * Columns (A-G):
 *   A: * Risk Name
 *   B: * Risk Category
 *   C: * Other Risk Category Name
 *   D: * Risk Description
 *   E: * Risk Impact
 *   F: * Risk Mitigation
 *   G: * Risk Status
 *
 * Applied via: applyRiskIdentificationHandler(CASPopulator) in populator.ts
 */
...
export function applyRiskIdentificationHandler(cls: any): void {

cls.prototype.handleRiskIdentificationPage = async function(): Promise<void> {
  if (!this.page) return;

  console.log('\n   📋 Processing Risk Identification page...');
  ...
  try {
    ...
    while (safetyLimit-- > 0) {
      ...
      if (!confirmed) {
        ...
        await this.page.goto(
          `${CONFIG.casBaseUrl}/appraisals/${CONFIG.appraisalId}/risk-identification`,
      ...
      if (!currentUrl.includes('risk-identification')) {
        await this.page.goto(
          `${CONFIG.casBaseUrl}/appraisals/${CONFIG.appraisalId}/risk-identification`,
          { waitUntil: 'networkidle2' }
        );
    ...
    // ── STEP 3: Load data from C_RiskIdentification ───────────────────────
    console.log('\n   === Step 3: Load C_RiskIdentification Data ===');

    const sourceWorkbook = new ExcelJS.Workbook();
    await sourceWorkbook.xlsx.readFile(CONFIG.excelFile);
    const sourceSheet = sourceWorkbook.getWorksheet('C_RiskIdentification');

    if (!sourceSheet) {
      console.log('   ⚠️  C_RiskIdentification sheet not found in source Excel');
      return;
    }
    ...
    // Row 1 = headers; data starts at row 2. 7 columns (A-G). Stop at first fully empty row.
    interface RiskRow {
      name: string;
      category: string;
    ...
    for (let rowNum = 2; rowNum <= sourceSheet.rowCount + 5; rowNum++) {
      ...
      dataRows.push({
        name:          vals[0],
        category:      vals[1],
        otherCategory: vals[2],
        description:   vals[3],
        impact:        vals[4],
        mitigation:    vals[5],
        status:        vals[6],
      });
      console.log(`      Row ${rowNum}: "${vals[0].substring(0, 45)}" [${vals[4]}] [${vals[6]}]`);
    }

    console.log(`   ✅ Loaded ${dataRows.length} risk entries`);

    if (dataRows.length === 0) {
      console.log('   ⚠️  No data in C_RiskIdentification - skipping upload');
      return;
    }
    ...
    try {
      ...
      templateHeaders = stringMatches.slice(0, 7).map((m: any) => {
        const match = m.match(/<(?:x:)?t[^>]*>([^<]*)<\/(?:x:)?t>/);
        return match ? match[1].replace(/\r\n/g, ' ').trim() : '';
      });
      console.log(`   Template headers (${templateHeaders.length}): ${templateHeaders.join(' | ')}`);
    } catch (e) {
      console.log(`   ⚠️  Could not extract template headers: ${e}`);
      templateHeaders = [
        '* Risk Name', '* Risk Category', '* Other Risk Category Name',
        '* Risk Description', '* Risk Impact', '* Risk Mitigation', '* Risk Status'
      ];
    }

    const outWorkbook = new ExcelJS.Workbook();
    const outSheet = outWorkbook.addWorksheet('Risk Identification');
    ...
    // Data rows - headings are the same in source and destination
    for (let i = 0; i < dataRows.length; i++) {
      const d = dataRows[i];
      const row = outSheet.getRow(i + 2);
      row.getCell(1).value = d.name;
      row.getCell(2).value = d.category;
      row.getCell(3).value = d.otherCategory;
      row.getCell(4).value = d.description;
      row.getCell(5).value = d.impact;
      row.getCell(6).value = d.mitigation;
      row.getCell(7).value = d.status;
      console.log(`      Row ${i + 2}: "${d.name.substring(0, 40)}"`);
    }
    ...
    console.log('\n   ✅ Risk Identification page processing complete');

  } catch (error) {
    console.log(`   ⚠️  Error processing risk identification: ${error}`);
    if (error instanceof Error) console.log(`   Stack: ${error.stack}`);
  }
}

} // end applyRiskIdentificationHandler
