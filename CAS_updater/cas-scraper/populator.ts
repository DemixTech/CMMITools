 ...
 *
 * Usage:
 *   1. Configure ../cas-project-config.json and ../.secrets/keys.json (relative to cas-scraper/)
 *   2. npm run populate
 *
 * Module structure:
...
import { applyResourceEstimatesHandler } from './handlers/resourceEstimates.handler';
import { applyLogisticalRequirementsHandler } from './handlers/logisticalRequirements.handler';
import { applyAppraisalConstraintsHandler } from './handlers/appraisalConstraints.handler';
import { applyRiskIdentificationHandler } from './handlers/riskIdentification.handler';
import { applyCOIHandler } from './handlers/conflictsOfInterest.handler';
...
class CASPopulator {
  ...
  async processCurrentPage(pagePath: string): Promise<PopulateLog> {
    ...
    if (pagePath === '/resource-estimates')                             { await this.handleResourceEstimatesPage();           return log; }
    if (pagePath === '/logistical-requirements')                         { await this.handleLogisticalRequirementsPage();      return log; }
    if (pagePath === '/appraisal-constraints')                           { await this.handleAppraisalConstraintsPage();        return log; }
    if (pagePath === '/risk-identification')                             { await this.handleRiskIdentificationPage();          return log; }
    if (pagePath === '/conflicts-of-interest')                           { await this.handleConflictsOfInterestPage();         return log; }
  ...
  async askForFeedback(): Promise<{ action: 'exit' | 'next' | 'reprocess'; feedback: string }> {
    if (!CONFIG.debugMode) {
      console.log('\n   ▶️  Debug mode OFF - auto-continuing to next page...');
      return { action: 'next', feedback: '' };
    }
    ...
    return new Promise((resolve) => {
      ...
      console.log('\nOptions:');
      console.log('  a               - Exit');
      console.log('  b  (or Enter)   - Continue to next page');
      console.log('  c               - Re-read current browser URL and process it');
      console.log('  [any text]      - Save as feedback, then continue to next page');
      console.log('  [any text] a    - Save feedback, then exit');
      console.log('  [any text] c    - Save feedback, then re-process current URL');
      console.log('');
      ...
      this.rl.question('Your input: ', (answer: string) => {
        ...
        // Bare command (no feedback text)
        if (inputLower === 'a' || inputLower === 'exit' || inputLower === 'quit' || inputLower === 'q' || inputLower === 's' || inputLower === 'stop') {
          resolve({ action: 'exit', feedback: '' });
          return;
        }
        if (input === '' || inputLower === 'b' || inputLower === 'continue' || inputLower === 'next') {
          resolve({ action: 'next', feedback: '' });
          return;
        }
        if (inputLower === 'c' || inputLower === 'reprocess' || inputLower === 'current' || inputLower === 'here') {
          resolve({ action: 'reprocess', feedback: '' });
          return;
        }

        // Feedback + optional trailing action: "...text... a" / "...text... c" / "...text... b"
        const trailingAction = input.match(/(?:^|\s)([abc])\s*$/i);
        let feedback = input;
        let action: 'exit' | 'next' | 'reprocess' = 'next';

        if (trailingAction) {
          feedback = input.slice(0, trailingAction.index).trim();
          const tail = trailingAction[1].toLowerCase();
          if (tail === 'a') action = 'exit';
          else if (tail === 'c') action = 'reprocess';
          else action = 'next';
        }

        // Backwards compatibility with legacy [s] / [q] suffix
        const legacyStop = /\[[sq]\]\s*$/i.test(input);
        if (legacyStop && !trailingAction) {
          feedback = input.replace(/\s*\[[sq]\]\s*$/i, '').trim();
          action = 'exit';
        }

        console.log(`💬 Feedback saved: "${feedback}"`);
        if (action === 'exit')      console.log('🛑 Exit requested after feedback.');
        if (action === 'reprocess') console.log('🔁 Re-process current URL requested after feedback.');
  ...
  // ── User interaction (manual-resume at dead-end) ─────────────────────────

  async askWhenStuck(reason: string): Promise<{ action: 'exit' | 'reprocess'; feedback: string }> {
    // Always prompts regardless of debugMode -- we've hit a dead end
    // (e.g. no Next button found). Lets the user drive the browser to any
    // page manually and tell the populator to process that page next.
    return new Promise((resolve) => {
      console.log('\n' + '='.repeat(60));
      console.log(`⚠️  ${reason}`);
      console.log('='.repeat(60));
      console.log('\nNavigate the browser to the page you want next, then choose:');
      console.log('\nOptions:');
      console.log('  a               - Exit');
      console.log('  c  (or Enter)   - Process the page currently in the browser');
      console.log('  [any text]      - Save as feedback, then process current page');
      console.log('  [any text] a    - Save feedback, then exit');
      console.log('');
      ...
      this.rl.question('Your input: ', (answer: string) => {
        ...
        if (input === '' || inputLower === 'c' || inputLower === 'continue' || inputLower === 'reprocess' || inputLower === 'current' || inputLower === 'here') {
          resolve({ action: 'reprocess', feedback: '' });
          return;
        }

        const trailingAction = input.match(/(?:^|\s)([ac])\s*$/i);
        let feedback = input;
        let action: 'exit' | 'reprocess' = 'reprocess';
        if (trailingAction) {
          feedback = input.slice(0, trailingAction.index).trim();
          action = trailingAction[1].toLowerCase() === 'a' ? 'exit' : 'reprocess';
        }
        console.log(`💬 Feedback saved: "${feedback}"`);
        if (action === 'exit') console.log('🛑 Exit after feedback.');
        resolve({ action, feedback });
      });
    });
  }
    ...
    try {
      ...
      while (continueProcessing) {
        ...
        if (shouldSkip) {
          ...
          if (!hasNextPage) {
            const { action } = await this.askWhenStuck('No Next button found after skipped page.');
            if (action === 'exit') {
              console.log('\n🛑 Exiting as requested.');
              continueProcessing = false;
            }
            // else 'reprocess': loop will re-read browser URL and process it
          } else {
            await new Promise(resolve => setTimeout(resolve, 2000));
          }
          continue;
        ...
        const { action, feedback } = await this.askForFeedback();
        ...
        if (action === 'reprocess') {
          // Skip clickNextButton; the next loop iteration will re-read the URL
          // (which may have changed if the user navigated manually) and process it.
          console.log('\n🔁 Re-reading current browser URL on next iteration...');
          await new Promise(resolve => setTimeout(resolve, 500));
          continue;
        }
        ...
        if (!hasNextPage) {
          const { action, feedback } = await this.askWhenStuck('No Next button found on this page.');
          if (feedback) {
            log.userFeedback = (log.userFeedback ? log.userFeedback + ' | ' : '') + feedback;
            this.saveLogs();
          }
          if (action === 'exit') {
            console.log('\n🛑 Exiting as requested.');
...
applyResourceEstimatesHandler(CASPopulator);
applyLogisticalRequirementsHandler(CASPopulator);
applyAppraisalConstraintsHandler(CASPopulator);
applyRiskIdentificationHandler(CASPopulator);
applyCOIHandler(CASPopulator);
...
interface CASPopulator {
  ...
  handleResourceEstimatesPage(): Promise<void>;
  handleLogisticalRequirementsPage(): Promise<void>;
  handleAppraisalConstraintsPage(): Promise<void>;
  handleRiskIdentificationPage(): Promise<void>;
  handleConflictsOfInterestPage(): Promise<void>;
}
