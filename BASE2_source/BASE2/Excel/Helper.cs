// Accessing Excel from .Core 3.1
// https://social.msdn.microsoft.com/Forums/vstudio/en-US/690930f1-7856-4f5f-b073-6cf2c40baa19/microsoftofficeinteropexcel-with-net-core?forum=netfxbcl
// https://docs.devexpress.com/OfficeFileAPI/401532/use-office-file-api-on-mac-os

// A solution for Excel on Windows and MAC OS is to use something like GemBox (see below)
// https://www.gemboxsoftware.com/spreadsheet/examples/c-sharp-microsoft-office-interop-excel-automation/6004
...
namespace BASE2
{
    static public class Helper
    {
        ...
        public static int FindEndOfWorksheet(Worksheet aWks, int columToSearch, int firstRow=1, int lastRow=50000)
        {
            ...
            var cellStr = aWks.Cells[centerX, columToSearch].ToString();// .Value;
        ...
        public static int FindEndOfWorksheetBrute(Worksheet aWks, int columToSearch, int firstRow = 1, int lastRow = 50000)
        {
            ...
            {
                if (aRangeToSearch[currentRow,1]?.ToString() != null)
                {
                    thresHold = 0;
           ...
           // Worksheet practiceAreaWks = questionWkb.Worksheets[worksheetName];
            foreach (Worksheet paWks in questionWkb.Worksheets)
            {
                if (paWks.Name.ToLower() == worksheetName.ToLower())
                {
                    ...
                    // Read the name
                    aNewPracticeArea.Name = paWks.Cells[1, 1].ToString();//.Value;
                    aNewPracticeArea.NameChinese = paWks.Cells[2, 1].ToString();//.Value;

                    // Read the intent and value statements
                    aNewPracticeArea.Intent = paWks.Cells[3, cPA_wks_IntentColumn].ToString();//.Value;
                    aNewPracticeArea.IntentChinese = paWks.Cells[4, 2].ToString();//.Value;
                    aNewPracticeArea.Value = paWks.Cells[5, 2].ToString();//.Value;
                    aNewPracticeArea.ValueChinese = paWks.Cells[6, cPA_wks_ValueColumn].ToString();//.Value;

                    // Find the end of the sheet and process each row
                    for (int row=cPAwksStartRow; row<= workSheetRows; row++)
                    {
                        string cellStr = paWks.Cells[row, cPA_wks_AcronymColumn].ToString();//.Value;
        ...
        public static Practice ProcessPractice(Worksheet paWks, int row, string firstColStr)
        {
            ...
            string practiceStatement = paWks.Cells[row, 2].ToString();//.Value;
            ...
            string exampleActivityStr = paWks.Cells[row, 3]?.ToString() ?? null;
            ...
            string exampleWorkProductsStr = paWks.Cells[row, 4]?.ToString() ?? null;
            ...
            string questions = paWks.Cells[row, 5]?.ToString() ?? null;
        ...
        public static Worksheet OpenOrElseCreateWks(Workbook aWkb, string wksNameToOpen)
        {
            ...
            Worksheet aNewWks = (Worksheet)aWkb.Worksheets.Add();
